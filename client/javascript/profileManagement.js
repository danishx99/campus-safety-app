...
function saveProfile() {
    ...
    let updateData = {};
        ...
        if(!cellCheck(phone)){
            showMessage("Invalid phone number. Please enter a valid 10-digit phone number.", true, 5000);
            return;
        }
    ...
    if (newPassword || confirmPassword) {
        ...
        if (!passwordPattern.test(password)) {
            if (!/(?=.*\d)/.test(password)) {
                ...
                showMessage("Password must contain at least one lowercase letter.", true, 5000);
            } else if (!/(?=.*[A-Z])/.test(password)) {
                showMessage("Password must contain at least one uppercase letter.", true, 5000);
            } else if (!/(?=.*[!@#$%^&*])/.test(password)) {
                showMessage("Password must contain at least one special character from the set [!@#$%^&*].", true, 5000);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('profileRole').textContent = role.charAt(0).toUpperCase() + role.slice(1).toLowerCase();
    document.getElementById('cellphone').value = cellphone
    document.getElementById('dateJoined').value = getDatePart(decodeURIComponent(dateJoined));
