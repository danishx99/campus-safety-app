      ...
      <div class="border border-black rounded-lg p-4 flex flex-col bg-white shadow-md">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
          ...
          <div class="flex items-center mt-2 sm:mt-0">
            <a href='https://www.google.com/maps?q=${location.latitude},${location.longitude}' target='_blank' class="mr-2">
            ...
            ${isSearching ? `<span class="searching-animation">Searching <span class="dot-animation"></span></span>` : alert.status}
            </p>
          </div>
        </div>
        <p class="mt-2 text-sm">${alert.description || 'No description provided'}</p>
          ${isSearching ? "" :` <p class="mt-1 text-xs">Assigned to: ${alert.assignedTo}</p>`}

        <p class="mt-1 text-xs">Reported at: ${new Date(alert.createdAt).toLocaleString()}</p>
...
function addSearchingAnimation() {
  ...
  async function displayEmergencyAlerts() {
    ...
    try {
      ...
      if (alerts.length === 0) {
        alertsContainer.innerHTML = '<p class="text-center text-gray-500">No emergency alerts found.</p>';
      } else {
      ...
      alertBox.textContent = 'Error fetching emergency alerts. Please try again later.';
