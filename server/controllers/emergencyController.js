...
exports.sendPanic = async (req, res) => {
    try {
        ...
        fcmTokens = users.map(user => user.FCMtoken);
         ...
         //Send notification
         await _sendNotification(fcmTokens, "Emergency Alert", "Emergency Alert received, click for more details", { notificationType: "emergency-alert", sender: email, senderLocation: location, recipient: "admin"});
